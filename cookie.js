...
(function () {
    'use strict';
     ...
     // Access the main cookie database
    window.cookieDatabase = window.cookieDatabase || {};
...
/* ===================== BUILD LISTS FROM MAIN DATABASE ===================== */
// We get all cookie data from your main cookieDatabase

// Domain mapping - which websites belong to which categories
const domainCategoryMap = {
    ...
    'clicktale.com': 'analytics',
    'usabilla.com': 'analytics',
    'qualaroo.com': 'analytics',
    'medallia.com': 'analytics',
    ...
    'ads.snapchat.com': 'advertising',
    'twitter.com': 'advertising',
    'ads.twitter.com': 'advertising',
    'reddit.com': 'advertising',
    'ads.reddit.com': 'advertising',
    'youtube.com': 'advertising',
    'ads.youtube.com': 'advertising',
    'google.com/ads': 'advertising',
    'adroll.com': 'advertising',
    'mailchimp.com': 'advertising',
    'activecampaign.com': 'advertising',
    'klaviyo.com': 'advertising',
    
    // Performance domains
    'cloudflare.com': 'performance',
    'akamaihd.net': 'performance',
    'edgekey.net': 'performance',
    'cdnjs.cloudflare.com': 'performance'
    'fastly.net': 'performance',
    'cdn77.org': 'performance',
    'keycdn.com': 'performance',
    'bunny.net': 'performance',
    'stackpath.com': 'performance'
};

// Create empty lists
const ANALYTICS_DATA = { domains: [], cookies: [] };
const MARKETING_DATA = { domains: [], cookies: [] };
const PERFORMANCE_DATA = { domains: [], cookies: [] };

// Fill the lists from your main database
(function() {
    // Access your main cookie database (it's at the bottom of your script)
    const mainDatabase = window.cookieDatabase || {};
    
    // Go through all cookies in database
    for (const [cookieName, cookieInfo] of Object.entries(mainDatabase)) {
        const category = cookieInfo.category;
        
        // Add to appropriate list
        if (category === 'analytics' || category === 'advertising' || category === 'performance') {
            const list = category === 'analytics' ? ANALYTICS_DATA : 
                        category === 'advertising' ? MARKETING_DATA : 
                        PERFORMANCE_DATA;
            
            if (!list.cookies.includes(cookieName)) {
                list.cookies.push(cookieName);
            }
        }
    }
    
    // Add domains from our mapping
    for (const [domain, category] of Object.entries(domainCategoryMap)) {
        const list = category === 'analytics' ? ANALYTICS_DATA : 
                    category === 'advertising' ? MARKETING_DATA : 
                    category === 'performance' ? PERFORMANCE_DATA : null;
        
        if (list) {
            list.domains.push(domain);
            // Also add www. version
            list.domains.push(`www.${domain}`);
        }
    }
    
    console.log("🔄 Blocking lists created from main database:");
    console.log("Analytics cookies:", ANALYTICS_DATA.cookies.length, "items");
    console.log("Advertising cookies:", MARKETING_DATA.cookies.length, "items");
    console.log("Performance cookies:", PERFORMANCE_DATA.cookies.length, "items");
})();
    ...
    for (const [domain, category] of Object.entries(domainCategoryMap)) {
        if (url.includes(domain)) {
            ...
            if (!getCategoryConsent(category)) {
                console.log(`🛡️ Blocked ${category} Domain: ${url}`);
                return true;
            }
            return false; // User consented, don't block
        }
    }
...
function shouldBlockCookie(cookieName) {
    ...
    // Get category from main database
    const mainDatabase = window.cookieDatabase || {};
    const cookieInfo = mainDatabase[cookieName];
    
    // Check if cookie is in our database
    if (cookieInfo) {
        const category = cookieInfo.category;
        
        // Check if user consented to this category
        if (!getCategoryConsent(category)) {
            console.log(`🛡️ Blocked ${category} Cookie: ${cookieName} (from database)`);
            return true;
        }
        return false;
    }
    
    // If not in database, check our lists (for partial matches)
    if (!getCategoryConsent('analytics')) {
        for (const cookie of ANALYTICS_DATA.cookies) {
            if (cookieName.includes(cookie) || cookie.includes(cookieName)) {
                console.log(`🛡️ Blocked Analytics Cookie (partial match): ${cookieName}`);
                return true;
            }
        }
    }
    ...
    if (!getCategoryConsent('advertising')) {
        for (const cookie of MARKETING_DATA.cookies) {
            if (cookieName.includes(cookie) || cookie.includes(cookieName)) {
                console.log(`🛡️ Blocked Marketing Cookie (partial match): ${cookieName}`);
                return true;
            }
        }
    }
    ...
    if (!getCategoryConsent('performance')) {
        for (const cookie of PERFORMANCE_DATA.cookies) {
            if (cookieName.includes(cookie) || cookie.includes(cookieName)) {
                console.log(`🛡️ Blocked Performance Cookie (partial match): ${cookieName}`);
                return true;
            }
        }
    }
